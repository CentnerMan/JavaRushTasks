...
public class Solution {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ArrayList<Character> oddChars = new ArrayList<>(); // гласные
        ArrayList<Character> Chars = new ArrayList<>(); // все остальные
        for (int i = 0; i < unformattedString.length(); i++) {
            Character testChar = unformattedString.charAt(i);
            if (isVowel(testChar)) {
                oddChars.add(testChar);
            } else if (testChar != (' ')) {
                Chars.add(testChar);
            }
        }
        for (int i = 0; i < oddChars.size(); i++) {
            System.out.print(oddChars.get(i) + " ");
        }
        System.out.println("");
